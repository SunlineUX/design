...
(function (global) {
  ...
  global.ViewComponents.TableView = {
    setup() {
      ...
      const cTable = `<x-table :columns="columns" :data="data" stripe>
  <template #col-status="{ row }">
    <x-badge :variant="statusColor(row.status)">{{ row.status }}</x-badge>
  </template>
</x-table>`;
      return { t, columns, data, statusColor, cTable };
    },
    template: `
      <div class="view-page">
        ...
        <x-waterfall :columns="1" :gap="16">
          <x-demo-block title="基础表格" :code="cTable">
          ...
          <x-demo-block title="空数据" :code="`<x-table :columns="columns" :data="[]" />`">
